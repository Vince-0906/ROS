...
tags:
  ...
  - ROS2
---
⬅️ **上一节**：[4.4.2 订阅参数更新](4.4.2%20订阅参数更新.md)
...
```python
        ...
        param.value = param_value
```
第一行是`param_value.type`
第一次中我用的是`param.value.type`，但下一行又用 `param_value` 对象整体替换了`param.value`，导致设置的 type 被覆盖丢失了。
此外，在main中的末尾需要使用同步处理，使用异步可能会导致第二次请求修改参数的动作会早于第一次检测图像的请求，也就是参数修改失败，两次用的同一个参数
```python
    node.update_detect_model('cnn') # cnn
    future = node.send_request()
    rclpy.spin_until_future_complete(node, future)
...
最终可以看到cnn和dog的显著差异
```bash
[INFO] [1785499126.208166547] [face_detect_client_node]: 人脸检测客户端已启动
[INFO] [1785499126.493622414] [face_detect_client_node]: 设置参数成功,参数名:model,参数值:cnn
[INFO] [1785499126.493937670] [face_detect_client_node]: 设置参数结果: True, 
[INFO] [1785499154.171480071] [face_detect_client_node]: 接收到相应，共：3张人脸，耗时：27.6458秒
[INFO] [1785499154.172227304] [face_detect_client_node]: 设置参数成功,参数名:model,参数值:hog
[INFO] [1785499154.172482424] [face_detect_client_node]: 设置参数结果: True, 
[INFO] [1785499154.471308412] [face_detect_client_node]: 接收到相应，共：2张人脸，耗时：0.2850秒
```

---

➡️ **下一节**：[4.5.1 参数声明与设置](../C%2B%2B/4.5.1%20参数声明与设置.md)　·　📑 [返回笔记索引](../../README.md#笔记索引)
