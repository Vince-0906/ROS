...
tags:
  ...
  - ROS2
---
⬅️ **上一节**：[3.2.1 通过Topic创建小说](3.2.1%20通过Topic创建小说.md)

> 📂 **配套代码**：[`333/topic_ws/src/demo_py_topic/demo_py_topic/novel_sub_node.py`](../../333/topic_ws/src/demo_py_topic/demo_py_topic/novel_sub_node.py) · [`333/novel1.txt`](../../333/novel1.txt)
...
```
> [!attention]
> 需要注意的就是thread在py里需要手动启动，然后为了避免CPU长时间占用，写入休眠机制
# 使用mimo-v2.5-tts
```python
import base64
import os
...
➡️ **下一节**：[3.3.1 发布速度控制海龟画圆](../C%2B%2B/3.3.1%20发布速度控制海龟画圆.md)　·　📑 [返回笔记索引](../../README.md#笔记索引)
